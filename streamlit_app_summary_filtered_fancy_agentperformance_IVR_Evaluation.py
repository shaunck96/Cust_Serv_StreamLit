...
class Intellibot:
    def __init__(self, call_transcriptions_df, ivr_data):
    ...
    def display_menu(self):
        option = st.selectbox("Select an option", ["Topic Trends", "Agent Performance", "Transfer Calls Analysis", "IVR Performance"])
        return option

    def validate_option(self, option):
        if option not in ["Topic Trends", "Agent Performance", "Transfer Calls Analysis", "IVR Performance"]:
    ...
    def ivr_performance(self):
        st.header("IVR Performance")
        
        start_date = st.date_input("Select a start date")
        end_date = st.date_input("Select an end date")

        self.ivr['Start_of_Call'] = pd.to_datetime(self.ivr['Start_of_Call'])
        start_date = pd.to_datetime(start_date).tz_localize('UTC')  # Convert start_date to the same datetime format
        end_date = pd.to_datetime(end_date).tz_localize('UTC')
        # Filter the DataFrame based on the selected dates and skill group
        filtered_df = self.ivr[
            (self.ivr['Start_of_Call'] >= start_date) &
            (self.ivr['Start_of_Call'] <= end_date)
        ]

        st.subheader("Intent Category Distribution")
        intent_distribution = pd.DataFrame(filtered_df.groupby(['Intent'])['call_sid'].nunique().sort_values(ascending=False)).reset_index()
        intent_distribution.columns = ['Intent', 'Count']
        # Create a Plotly bar chart
        fig = px.bar(intent_distribution, x='Intent', y='Count',
                    labels={'x': 'Intent Category', 'y': 'Number of Calls'},
                    title="Intent Category Distribution")
        st.plotly_chart(fig)

        st.subheader("Transferred Calls: Intent Category Distribution")
        transferred_intent_distribution = pd.DataFrame(filtered_df[filtered_df['segmentTransferredYN']=='Y'].groupby(['Intent'])['call_sid'].nunique().sort_values(ascending=False)).reset_index()
        transferred_intent_distribution.columns = ['Transferred_Intent', 'Count']
        # Create a Plotly bar chart
        fig = px.bar(transferred_intent_distribution, x='Transferred_Intent', y='Count',
                    labels={'x': 'Transferred Calls: Intent Category', 'y': 'Number of Calls'},
                    title="Transferred Calls: Intent Category Distribution")
        st.plotly_chart(fig)
    ...
    def run(self):
        ...
        if self.validate_option(option):
            ...
            elif option == "IVR Performance":
                self.ivr_performance()
...
if __name__ == '__main__':
    ...
    bot = Intellibot(call_transcriptions_df,call_transcriptions_df)
    bot.run()
